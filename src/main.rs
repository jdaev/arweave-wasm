...
use sha2::{digest::generic_array::ArrayLength, Sha256};
use std::fs;
...
fn main() {
    // Create a ChaChaRng random number generator using the seed
    let mut seed =
    ...
    println!(
        "📁 Output directory: {}",
        &outdir.to_slash_lossy().to_string().yellow()
    );
    ...
    for (t, wallet) in rx {
        ...
        if pattern.is_match(address) {
            ...
            println!(
                "🏁 Done in {:.3}s {}",
                el.to_string().bold(),
                format!("({:.1}/s)", (count as f32 / el)).dimmed()
            );
